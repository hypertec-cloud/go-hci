...
func BMTestChangeComputeOfferingReturnTrueIfSuccess(t *testing.T) {
	//given
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	baremetalWithNewComputeOffering := Baremetal{
		Id:                TEST_BAREMETAL_ID,
		ComputeOfferingId: "new_compute_offering",
	}

	mockEntityService.EXPECT().Execute(TEST_BAREMETAL_ID, BAREMETAL_CHANGE_COMPUTE_OFFERING_OPERATION, gomock.Any(), gomock.Any()).Return([]byte(`{}`), nil)

	//when
	success, _ := baremetalService.ChangeComputeOffering(baremetalWithNewComputeOffering)

	//then
	assert.True(t, success)
}

func BMTestChangeComputeOfferingReturnFalseIfError(t *testing.T) {
	//given
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	mockEntityService.EXPECT().Execute(TEST_BAREMETAL_ID, BAREMETAL_CHANGE_COMPUTE_OFFERING_OPERATION, gomock.Any(), gomock.Any()).Return([]byte(`{}`), mockError)
	...
	//when
	success, err := baremetalService.ChangeComputeOffering(baremetalWithNewComputeOffering)
...
func BMTestResetPasswordReturnNewPasswordIfSuccess(t *testing.T) {
	//given
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	mockEntityService.EXPECT().Execute(TEST_BAREMETAL_ID, BAREMETAL_RESET_PASSWORD_OPERATION, gomock.Any(), gomock.Any()).Return([]byte(`{"password":"new_password"}`), nil)

	//when
	newPassword, _ := baremetalService.ResetPassword(TEST_BAREMETAL_ID)

	//then
	assert.Equal(t, "new_password", newPassword)
}

func BMTestResetPasswordReturnEmptyStringIfError(t *testing.T) {
	//given
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	mockEntityService.EXPECT().Execute(TEST_BAREMETAL_ID, BAREMETAL_RESET_PASSWORD_OPERATION, gomock.Any(), gomock.Any()).Return([]byte(`{}`), mockError)

	//when
	newPassword, err := baremetalService.ResetPassword(TEST_BAREMETAL_ID)
...
func BMTestCreateRecoveryPointReturnTrueIfSuccess(t *testing.T) {
	//given
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	mockEntityService.EXPECT().Execute(TEST_BAREMETAL_ID, BAREMETAL_CREATE_RECOVERY_POINT_OPERATION, gomock.Any(), gomock.Any()).Return([]byte(`{}`), nil)

	//when
	success, _ := baremetalService.CreateRecoveryPoint(TEST_BAREMETAL_ID, RecoveryPoint{"new_recovery_point_name", "description"})

	//then
	assert.True(t, success)
}

func BMTestCreateRecoveryPointReturnFalseIfError(t *testing.T) {
	//given
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	...
	mockEntityService.EXPECT().Execute(TEST_BAREMETAL_ID, BAREMETAL_CREATE_RECOVERY_POINT_OPERATION, gomock.Any(), gomock.Any()).Return([]byte(`{}`), mockError)

	//when
	success, err := baremetalService.CreateRecoveryPoint(TEST_BAREMETAL_ID, RecoveryPoint{"new_recovery_point_name", "description"})
